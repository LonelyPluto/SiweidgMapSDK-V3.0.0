...
public class RuleBuilder {
    ...
    public Rule onComplete(int[] level) {
        ...
        if (styleBuilder != null)
            for (RenderStyle.StyleBuilder<?> style : styleBuilder) {
        ...
        if (type != RuleType.POSITIVE)
            return new Rule.NegativeRule(type, element, zoom, selector, keys, values, rules, styles)
                    .setCat(cat);

        if (numKeys == 1 && numVals == 0)
            return new Rule.PositiveRuleK(element, zoom, selector, keys[0], rules, styles)
                    .setCat(cat);

        if (numKeys == 0 && numVals == 1)
            return new Rule.PositiveRuleV(element, zoom, selector, values[0], rules, styles)
                    .setCat(cat);

        if (numKeys == 1 && numVals == 1)
            return new Rule.PositiveRuleKV(element, zoom, selector, keys[0], values[0], rules, styles)
                    .setCat(cat);

        return new Rule.PositiveRuleMultiKV(element, zoom, selector, keys, values, rules, styles)
                .setCat(cat);
    }
