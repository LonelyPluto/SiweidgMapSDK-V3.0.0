...
public class UrlTileDataSource implements ITileDataSource {
    ...
    public UrlTileDataSource(UrlTileSource tileSource, ITileDecoder tileDecoder, HttpEngine conn) {
        ...
        mUseCache = (tileSource.getCache() != null);
        mConn = conn;
    }

    @Override
    public void query(MapTile tile, ITileDataSink sink) {
        tile.setMapType(mTileSource.getMapType());

        // 缓存
        ITileCache cache = mTileSource.getCache();
        if (mUseCache) {
        ...
        // 本地db文件
        IMapTileProvider mapTileProvider = mTileSource.getMapTileFileProvider();
        if (mapTileProvider != null) {
            InputStream is = mapTileProvider.getInputStream(tile);
            if (is != null) {
                try {
                    if (mTileDecoder.decode(tile, sink, is)) {
                        sink.completed(SUCCESS);
