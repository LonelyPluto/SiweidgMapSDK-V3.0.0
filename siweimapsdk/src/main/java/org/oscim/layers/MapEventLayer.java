...
import org.oscim.event.MapViewListener;
import org.oscim.event.MotionEvent;
import org.oscim.event.OnMapMoveListener;
import com.siweidg.siweimapsdk.map.Animator2;
import com.siweidg.siweimapsdk.map.SiweidgMap;
import com.siweidg.siweimapsdk.map.SiweidgMap.InputListener;
import com.siweidg.siweimapsdk.map.ViewController;
 ...
 */
public class MapEventLayer extends AbstractMapEventLayer implements InputListener, GestureListener, SiweidgMap.UpdateListener {
    ...
    public MapEventLayer(SiweidgMap map) {
        super(map);
    ...
    private void onActionMove(MotionEvent e) {
        ...
        if (e.getPointerCount() < 2) {
            ...
            if (mMap.viewport().getMapPosition(mapPosition)) {
                mMap.events.fire(SiweidgMap.MOVE_EVENT, mapPosition);
            }
            return;
        }
        mStartMove = -1;
        ...
        if (mMap.viewport().getMapPosition(mapPosition)) {
            if (mDoScale)
                mMap.events.fire(SiweidgMap.SCALE_EVENT, mapPosition);
            if (mDoRotate)
                mMap.events.fire(SiweidgMap.ROTATE_EVENT, mapPosition);
            if (mDoTilt)
                mMap.events.fire(SiweidgMap.TILT_EVENT, mapPosition);
        }
    }
    ...
    public boolean onGesture(Gesture g, MotionEvent e) {
        ...
        if (listener != null) {
            if (g instanceof Gesture.Tap) {
                return listener.onSingleTap(geoPoint);
            } else if (g instanceof Gesture.LongPress) {
                return listener.onLongPress(geoPoint);
            }
        }
        return false;
    }
    ...
    public void onMapEvent(Event e, MapPosition mapPosition) {
        ...
        if (onMapMoveListener != null){
            onMapMoveListener.onMapMove(mapPosition);
        }
    }
